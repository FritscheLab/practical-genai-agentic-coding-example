...
# ==============================================================================
# Script: reference.R
# Description: Plot fixed, invented measurement counts for two groups.
# Author: Not recorded
# Date: Not recorded
# Input: --output followed by the destination PNG path.
# Output: PNG chart; no console output on success.
# ==============================================================================

# --- ⚙️ Constants -------------------------------------------------------------
summary_categories <- c(
...
summary_counts <- matrix(
  c(42, 31, 18, 9, 64, 18, 12, 6), nrow = 2, byrow = TRUE,
  dimnames = list(summary_groups, summary_categories)
)

# --- 📊 Plotting function -----------------------------------------------------
plot_summary <- function(output_path) {
  # These totals are invented constants; the function reads no data files.
  if (length(output_path) != 1L || is.na(output_path) ||
      !nzchar(output_path) || !endsWith(tolower(output_path), ".png")) {
  ...
  png(
    filename = output_path, width = 768, height = 456, res = 120,
  ...
  for (group in seq_along(summary_groups)) {
    ...
    positions <- barplot(
      counts, names.arg = rev(summary_categories),
      horiz = TRUE, las = 1, col = "#0f766e", border = NA,
      xlim = c(0, 80), xlab = "Invented measurement count",
      main = "Measurement completeness (invented totals)",
      add = group > 1
    )
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (length(args) != 2L || args[[1L]] != "--output" ||
      !nzchar(args[[2L]]) || startsWith(args[[2L]], "--") ||
